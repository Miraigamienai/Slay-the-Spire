...
namespace Monster{
    SpikeSlimeM::SpikeSlimeM(float offsetX, float offsetY):Monsters(Setting::WINDOW_WIDTH*0.75F+offsetX, FLOOR_Y+offsetY, WIDTH, HIGHT,HPBarWidth)
    {
        setHP(MIN_HP,MAX_HP);
        setBlock(0);
    ...
    void SpikeSlimeM::Action(Dungeon::Dungeon_shared &dungeon_shared){
        if(ActionCount>=2) {
            ...
            currentAction=static_cast<Monster::SpikeSlimeMAction>(dungeon_shared.random_package.monster_ai_rng.GetRandomWithWeight(ActionProbability,sizeof(ActionProbability)/sizeof(float)));
    ...
    void SpikeSlimeM::render(const std::shared_ptr<Draw::Draw_2D> &r2) const 
    {
        if(IsFadeOut){
    ...
    const std::shared_ptr<Draw::ReTexture> &SpikeSlimeM::img=RUtil::Image_book::GetTexture(RESOURCE_DIR"/Image/monster/Spike Slime/Spike-slime-l-pretty.png");
